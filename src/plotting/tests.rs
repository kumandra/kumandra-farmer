...
use kumandra_solving::{create_tag, KumandraCodec};
...
async fn plotting_happy_path() {
    ...
    let kumandra_codec = KumandraCodec::new_with_gpu(identity.public_key().as_ref());
...
async fn plotting_piece_eviction() {
    ...
    let archiving_instance = Archiving::start(
        ...
        object_mappings,
        client.clone(),
        plotting::plot_pieces(kumandra_codec.clone(), &plot, commitments.clone()),
    )
    .await
    .unwrap();
